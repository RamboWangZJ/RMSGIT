...
@interface CreatTelViewController ()

@end

@implementation CreatTelViewController
{
    UILabel *_Tel;
...
-(void)initUI{
    ...
    _barView.titleLab.text=NSLocalizedString(@"xuanzetel_lg",nil);
...
-(void)textFieldDidBeginEditing:(UITextField *)textField{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        //执行的动画
        _backScroll.contentOffset = CGPointMake(0, 60);
        
        CGFloat y=_lineone.frame.origin.y + _lineone.frame.size.height + 150*HSCALE;
        _nextBtn.frame =CGRectMake(SCREEN_WIDTH-48*WSCALE-102*WSCALE, y, 102*WSCALE, 102*HSCALE);
...
-(void)telCheck{
    ...
    CGFloat y=SCREEN_HEIGHT-24*HSCALE-102*HSCALE;
    _nextBtn.frame =CGRectMake(SCREEN_WIDTH-48*WSCALE-102*WSCALE, y, 102*WSCALE, 102*HSCALE);
    CreateTelCheckViewController *vc=[[CreateTelCheckViewController alloc]init];
    [self.navigationController pushViewController:vc animated:YES];
}


-(void)nextAction{
    BOOL isPhoneChanged;//是否做了更改
    if ([[DataProcessing sharedManager].phone isEqualToString:_TelTextField.text]) {
        isPhoneChanged=NO;
    }else{
        isPhoneChanged=YES;
    }
    ...
    [DataProcessing sharedManager].phone=_TelTextField.text;
    [DataProcessing sharedManager].country=_telZoneButton.titleLabel.text;
    ...
    //60秒之内 没有改
    if ([[NSUserDefaults standardUserDefaults]integerForKey:CREATSECOND]>0 && !isPhoneChanged) {
        ...
        dic=@{@"country_code":_telZoneButton.titleLabel.text,@"mobile":_TelTextField.text,@"use_case":@10};
        ...
        [[httpManager sharedManager]httpWithURL:@"util/sendsms" method:GET param:dic timeString:dateStr sign:sign success:^(NSDictionary *responseDic) {
            ...
            if (status==1){
                ...
                [[NSUserDefaults standardUserDefaults]setInteger:59 forKey:CREATSECOND];
                [self.navigationController pushViewController:vc animated:YES];
