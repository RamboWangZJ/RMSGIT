...
-(void)initUI{
    ...
    [_infoLab mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(SCREEN_WIDTH-110*WSCALE);
    }];
...
-(void)zidongDaojishi{
    ...
    //默认是0
    if ([[NSUserDefaults standardUserDefaults]integerForKey:CREATSECOND]==0) {
        time = 59;
    }else{
        time = [[NSUserDefaults standardUserDefaults]integerForKey:CREATSECOND];
    }
    ...
    dispatch_source_set_event_handler(_timer, ^{
        ...
        [[NSUserDefaults standardUserDefaults]setInteger:time forKey:CREATSECOND];
        ...
        if(time <= 0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [[NSUserDefaults standardUserDefaults]setInteger:0 forKey:CREATSECOND];
                //结束 设置按钮的样式
...
-(void)fasongYanzhengma{
    ...
    dic=@{@"country_code":_contrycode,@"mobile":_telStr,@"use_case":@10};
...
-(void)nextAction{
    NSDictionary *dic=[[NSDictionary alloc]init];
    dic=@{@"country_code":_contrycode,@"mobile":_telStr,@"use_case":@10,@"sms_code":_yanzhengTF.text};
    NSString *dateStr=[[DataProcessing sharedManager]nowTimestamp];
