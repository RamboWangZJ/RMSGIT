...
-(void)nextAction{
    ...
    NSDictionary *dic=@{@"country_code":[DataProcessing sharedManager].contry_reset,@"password":_passwordTextField.text,@"use_case":@30,@"type":@10,@"mobile":[DataProcessing sharedManager].phone_reset};
    
    NSString *dateStr=[[DataProcessing sharedManager]nowTimestamp];
    NSString *sign=[[DataProcessing sharedManager]dataSignWithParam:dic url:@"user/forgetpwdupdate"timeString:dateStr];
    [[httpManager sharedManager]httpWithURL:@"user/forgetpwdupdate" method:POST param:dic timeString:dateStr sign:sign success:^(NSDictionary *responseDic) {
        ...
        if (status==1){
            ...
            UIAlertView *alert=[[UIAlertView alloc]initWithTitle:NSLocalizedString(@"mimayishezhi_lg",nil) message:NSLocalizedString(@"mimayishezhichongxindl_lg",nil) delegate:self cancelButtonTitle:NSLocalizedString(@"queren",nil) otherButtonTitles:nil, nil];
            [alert show];
